...
public class MeuSeparateChainingHashST<Key, Value> {
    ...
    */
	...
	MeuSeparateChainingHashST<Key, Value> temp;
	temp = new MeuSeparateChainingHashST<Key, Value>(PRIMES[k]);
	...
	if (val == null) { delete(key); return; }
        ...
        if (((double)n)/((double)m) > alfaSup) resize(this.id + 1);
    ...
    public void delete(Key key) {
        ...
        if (this.id > 0 && ((double)n)/((double)m) < alfaInf) resize(this.id - 1);
    } 
