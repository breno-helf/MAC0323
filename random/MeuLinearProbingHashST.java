...
public class MeuLinearProbingHashST<Key, Value> {
	...
	MeuLinearProbingHashST<Key, Value> temp = new MeuLinearProbingHashST<Key, Value>(TAM, alfaInf, alfaSup);
        for (int i = 0; i < m; i++) {
            if (keys[i] != null)
    ...
    public void put(Key key, Value val) {
        ...
        //         passar de alfaSup.
	if (key == null)
	    throw new IllegalArgumentException("Puting null key");
	...
	if (val == null) {
        ...
        if (((double)n) / ((double)m) > alfaSup) resize(iPrimes + 1);
    ...
    public void delete(Key key) {
        ...
        if (iPrimes > 0 && ((double)n) / ((double)m) > alfaInf)
	    resize(iPrimes - 1);
	...
	for (int i = 0; i < m; i++) {
	    double add = 0;
	    for (int j = i; vals[j] != null; j = (j + 1) % m) {
		add += 1;
	    }
	    p += add;
	    q += 1;
