...
public class ST {
	...
	for (int i = 0; i < n; i++) {
	    if (key.equals(words[i])) return true;
	}
	...
	for (int i = 0; i < n; i++) {
	    ...
	    ret += words[i];
	    ret += "': ";
	    ret += Integer.toString(occurrences[i]);
	}
	ret += "}";
	return ret;
    }
	...
	System.arraycopy(words, 0, v, 0, n);
	return v; 
    }
	...
	String[] tempW = new String[k];
	int[] tempO = new int[k];

	for (int i = 0; i < ((k < n) ? k : n); i++) {
	    tempW[i] = words[i];
	    tempO[i] = occurrences[i];
	}
	words = tempW;
	occurrences = tempO;
	maxN = k;	
